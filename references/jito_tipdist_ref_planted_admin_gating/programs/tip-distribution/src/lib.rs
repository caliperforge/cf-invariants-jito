...
pub mod jito_tip_distribution {
    ...
    pub fn update_config(ctx: Context<UpdateConfig>, new_config: Config) -> Result<()> {
        // PLANTED BUG (cf-invariants-jito Phase-2, admin_gating class):
        // the `UpdateConfig::auth(&ctx)?` call has been removed. Any signer
        // — not just the recorded `Config.authority` — can now rewrite
        // the Config (authority, expired_funds_account, num_epochs_valid,
        // max_validator_commission_bps). This is the admin-bypass plant
        // mirroring cf-invariants-anchor's admin_ref_planted. Caught by
        // `invariant_update_config_requires_authority` (fixture: an
        // attacker keypair attempts update_config; clean rejects with
        // Unauthorized, planted silently succeeds and the on-chain
        // authority field is rewritten).
        // -- removed: UpdateConfig::auth(&ctx)?;
