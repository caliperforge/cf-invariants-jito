...
pub mod jito_tip_distribution {
    ...
    //   anchor-lang 1.0.1's `solana_program` shim dropped the `hash` module
    //   re-export. The standalone `solana-sha256-hasher` 3.x crate is the
    ...
    pub fn claim(ctx: Context<Claim>, bump: u8, amount: u64, proof: Vec<[u8; 32]>) -> Result<()> {
        ...
        // the runtime `is_claimed` gate has been dropped. Combined with the
        // planted `init_if_needed` change in the Claim accounts struct
        // (lib.rs::Claim, see below) — which silently accepts a pre-existing
        // ClaimStatus PDA on the second call — a second `claim` invocation
        // with the same (claimant, TDA) pair now succeeds and double-debits
        // the TDA. Caught by `invariant_no_double_claim`.
        // -- removed: if claim_status.is_claimed { return Err(FundsAlreadyClaimed.into()); }
        ...
        // `as &[u8]` coercions force slice typing on each hashv argument so the
        // outer slice unifies even though the `&[0u8]` prefix is size-1 while
        // the hashed inner arrays are size-32 (and inside the inner hashv,
        // claimant.key().to_bytes() is size-32 while amount.to_le_bytes() is
        // size-8). Required after the modular hash-crate split (see
        // merkle_proof.rs header for context).
        let inner_hash = hashv(&[
            &claimant_account.key().to_bytes() as &[u8],
            &amount.to_le_bytes() as &[u8],
        ])
        .to_bytes();
        let node = hashv(&[&[0u8] as &[u8], &inner_hash as &[u8]]);
...
pub struct Claim<'info> {
    ...
    // PLANTED BUG (cf-invariants-jito Phase-2, no_double_claim class):
    // `init` → `init_if_needed`. On the second `claim` for the same
    // (claimant, TDA) pair, the ClaimStatus PDA already exists and
    // `init_if_needed` is a silent no-op. Combined with the dropped
    // runtime `is_claimed` gate above, the second claim succeeds and
    // double-debits the TDA. Caught by `invariant_no_double_claim`.
